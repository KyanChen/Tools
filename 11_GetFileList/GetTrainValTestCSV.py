...
class GetTrainTestCSV:
    ...
    def get_list(self, phase):
        ...
        if phase == 'seg':
            ...
            data_information['img'] = file_list
            data_information['labels'] = [x.replace('.tiff', '.txt') for x in file_list]
...
if __name__ == '__main__':
    data_path_list = [r'G:\Coding\YOLOv4_Refine\dataset\test_data']
    csv_name = 'val_GF.csv'
    img_format_list = ['tiff']
    negative_keep_rate = 0.1  # when meet negative samples, there is negative_rate to retain them.

    getTrainTestCSV = GetTrainTestCSV(dataset_path_list=data_path_list, csv_name=csv_name, img_format_list=img_format_list)
    getTrainTestCSV.get_list(phase='det')
    # getTrainTestCSV.generate_val_data_from_train_data(frac=0.2)
